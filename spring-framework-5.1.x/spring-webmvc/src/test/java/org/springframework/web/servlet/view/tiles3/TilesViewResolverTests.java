...
public class TilesViewResolverTests {
	...
	public void testResolve() throws Exception {
		given(this.renderer.isRenderable(eq("/prototype.test"), isA(Request.class))).willReturn(true);
		given(this.renderer.isRenderable(eq("/nonexistent.test"), isA(Request.class))).willReturn(false);

		assertTrue(this.viewResolver.resolveViewName("/prototype.test", Locale.ITALY) instanceof TilesView);
		assertNull(this.viewResolver.resolveViewName("/nonexistent.test", Locale.ITALY));

		verify(this.renderer).isRenderable(eq("/prototype.test"), isA(Request.class));
