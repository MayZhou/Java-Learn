...
[subs="verbatim,quotes"]
----
	package org.springframework.core.convert.com.mayzhou.runtimeAnnotations.excelkit.converter;
...
To create your own com.mayzhou.runtimeAnnotations.excelkit.converter, implement the `Converter` interface and parameterize `S`
as the type you are converting from and `T` as the type you are converting to. You can also transparently apply such a
com.mayzhou.runtimeAnnotations.excelkit.converter if a collection or array of `S` needs to be
converted to an array or collection of `T`, provided that a delegating array or collection
com.mayzhou.runtimeAnnotations.excelkit.converter has been registered as well (which `DefaultConversionService` does by default).
...
Several com.mayzhou.runtimeAnnotations.excelkit.converter implementations are provided in the `core.convert.support` package as
...
A good example of a `GenericConverter` is a com.mayzhou.runtimeAnnotations.excelkit.converter that converts between a Java array
...
no need to create a specific com.mayzhou.runtimeAnnotations.excelkit.converter to convert from a `Collection` of `S` to a
...
com.mayzhou.runtimeAnnotations.excelkit.converter and formatter registration.
